...
ficheroRCS<-read.csv(fname,stringsAsFactors = F,sep= "\t")
```
  
  3. Convertir fecha y hora con `as.POSIXlt` y `as.hms` (`hms` library).
```{r}

fechaHora<-paste(ficheroRCS$FECHA,ficheroRCS$HORA)
FechaHoraPOSIXlt<-as.POSIXlt(fechaHora, format="%d/%m/%Y %H:%M:%OS")

library(hms)
horasHMS<-as.hms(ficheroRCS$HORA)
```
...
save(file="data/Almacenamiento.Rdata",list = c('fechaHora'))
load(file="./data/Almacenamiento.Rdata")
...
BreadBasket_DMS <- read.csv("./data/BreadBasket_DMS.csv",sep= ",")
save(file="data/Almacenamiento.Rdata",list = c('BreadBasket_DMS'))
```
...
ficheroJson<-read_json(filename,simplifyVector = TRUE )
save(file="data/AlmacenamientoJson.Rdata",list = c('ficheroJson'))
```
...
library(jsonlite)
library(readr)
write_json(iris, "./iris.json", pretty = TRUE, dataframe = "rows", na = "null")
json_iris<-toJSON(iris,dataframe = "rows", pretty = TRUE, na = "null")
write_file(json_iris, "./iris_write_file.json")
...
df <- read_excel(
  "./data/ERCA.xlsx",
   sheet=2,
  col_types = c("numeric","numeric","date","text","date","date","numeric","numeric","numeric","date","guess","guess","guess","guess","guess","guess","guess","guess","guess","guess","guess","guess","guess","guess","guess","guess","guess","guess","guess","guess","guess","guess","guess","guess","guess","guess","guess","guess","guess","guess","guess","guess","guess","guess","guess","guess","guess","guess","guess","guess","guess","guess","guess","guess","guess","guess","guess","guess")
)
save(file="./data/ExcelData.Rdata",list = c('df'))
```
...
df_limpio <- df[!is.na(df$IDENTIFICA), ]
...
```{r}
filename <- "data/subjectInfo.xlsx"
measurment <- read_excel(filename, sheet = 1)
neuropathy <- read_excel(filename, sheet = 2)
tunning <- read_excel(filename, sheet = 3)
mobility <- read_excel(filename, sheet = 4)

todos<- excel_sheets(filename)
df <- list()
for (n in todos) {
  df[[n]] <- read_excel(path = filename, sheet = n)
}

divideLibro <- function(s) read_excel(path = filename, sheet = s)

x <- lapply(todos, divideLibro)
names(x) <- todos
save(x, file = "./data/Hojas.RData")
# save()
```
